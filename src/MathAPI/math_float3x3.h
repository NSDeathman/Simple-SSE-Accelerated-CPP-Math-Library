...
namespace Math
{
     ...
     * @note SSE optimized matrix multiplication
     */
    inline float3x3 operator*(const float3x3& lhs, const float3x3& rhs) noexcept
    {
        float3x3 result;

        // Используем прямой доступ к данным для максимальной скорости
        const float* l = &lhs.col0_.x;
        const float* r = &rhs.col0_.x;
        float* res = &result.col0_.x;

        // Развернутый цикл для каждого столбца результата
        for (int j = 0; j < 3; ++j) {
            __m128 sum = _mm_setzero_ps();

            // Внутренний цикл развернут для устранения ветвлений
            __m128 rhs_val0 = _mm_set1_ps(r[j * 4 + 0]); // r(0,j)
            __m128 lhs_col0 = _mm_load_ps(l + 0 * 4);     // lhs.col0
            sum = _mm_add_ps(sum, _mm_mul_ps(rhs_val0, lhs_col0));

            __m128 rhs_val1 = _mm_set1_ps(r[j * 4 + 1]); // r(1,j)  
            __m128 lhs_col1 = _mm_load_ps(l + 1 * 4);     // lhs.col1
            sum = _mm_add_ps(sum, _mm_mul_ps(rhs_val1, lhs_col1));

            __m128 rhs_val2 = _mm_set1_ps(r[j * 4 + 2]); // r(2,j)
            __m128 lhs_col2 = _mm_load_ps(l + 2 * 4);     // lhs.col2
    ...
    /**
     * @brief Compute normal matrix for transforming normals
     * @param model Model transformation matrix
     * @return Normal transformation matrix
     */
     ...
     * @param vec Vector to create matrix from
